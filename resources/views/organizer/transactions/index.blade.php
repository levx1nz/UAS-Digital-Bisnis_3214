...
@section('page_subtitle', 'Semua transaksi tiket dari event Anda.')
...
<div class="space-y-8">
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black text-slate-900">Rp {{ number_format($sumRevenue, 0, ',', '.') }}</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Transaksi Sukses</p>
            <h3 class="text-2xl font-black text-emerald-600">{{ number_format($countSuccess, 0, ',', '.') }}</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Pending</p>
            <h3 class="text-2xl font-black text-amber-600">{{ number_format($countPending, 0, ',', '.') }}</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Total Transaksi</p>
            <h3 class="text-2xl font-black text-slate-900">{{ number_format($countTotal, 0, ',', '.') }}</h3>
        </div>
    </div>

    <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <form action="{{ route('organizer.transactions.index') }}" method="GET" class="flex flex-wrap gap-4 items-center">
            ...
            @if(request('search') || request('status'))
                <a href="{{ route('organizer.transactions.index') }}" class="px-6 py-3 bg-red-50 text-red-600 border border-red-100 rounded-xl font-bold hover:bg-red-100 transition-colors">Reset</a>
            @endif
        </form>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y border-t">
                    ...
                    <tr class="hover:bg-slate-50/50 transition {{ $trx->status == 'pending' ? 'text-slate-400' : '' }}">
                        <td class="px-8 py-6">
                            <span class="font-mono font-bold px-3 py-1 rounded-lg text-sm {{ $trx->status == 'pending' ? 'bg-slate-100 text-slate-500' : 'text-indigo-600 bg-indigo-50' }}">{{ $trx->order_id }}</span>
                        </td>
                        <td class="px-8 py-6">
